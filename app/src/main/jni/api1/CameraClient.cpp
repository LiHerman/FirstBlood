...
status_t CameraClient::sendCommand(int32_t cmd, int32_t arg1, int32_t arg2) {
    ...
    int orientation = 0;
    char default_rotation[PROPERTY_VALUE_MAX];
    char front_value[PROPERTY_VALUE_MAX];
    char back_value[PROPERTY_VALUE_MAX];
    char front_rotation_value[PROPERTY_VALUE_MAX];
    char back_rotation_value[PROPERTY_VALUE_MAX];
    ...
    sprintf(default_rotation, "%d", arg1);
    Mutex::Autolock lock(mLock);
    status_t result = checkPidAndHardware();
    if (result != NO_ERROR) return result;
    ...
    if (cmd == CAMERA_CMD_SET_DISPLAY_ORIENTATION) {
        ...
        //orientation = getOrientation(arg1, true/*mCameraFacing == CAMERA_FACING_FRONT*/);
        property_get("persist.sys.frontcamera.mirror", front_value, "false");
        property_get("persist.sys.backcamera.mirror", back_value, "true");
        property_get("persist.sys.front.rotation", front_rotation_value, default_rotation);
        property_get("persist.sys.back.rotation", back_rotation_value, default_rotation);
        if((strcmp(front_value,"true") == 0) && (mCameraFacing == CAMERA_FACING_FRONT)){
            orientation = getOrientation(strtol(front_rotation_value,0,0), true);
        } else if((strcmp(back_value,"true") == 0) && (mCameraFacing == CAMERA_FACING_BACK)){
            orientation = getOrientation(strtol(back_rotation_value,0,0), true);
        } else {
            if(mCameraFacing == CAMERA_FACING_BACK){
                orientation = getOrientation(strtol(back_rotation_value,0,0), false);
            } else {
                orientation = getOrientation(strtol(front_rotation_value,0,0), false);
            }
        }
        ALOGE("CameraClient::sendCommand mCameraFacing(%d) orientation (%d) default_rotation (%d)",mCameraFacing,orientation,arg1);
