...
#include <system/camera_metadata.h>build-binary
...
{
    ALOGD("CameraService updateCameraNum");
...
Status CameraService::getNumberOfCameras(int32_t type, int32_t* numCameras) {
    ...
    switch (type) {
        case CAMERA_TYPE_BACKWARD_COMPATIBLE:
            ...
            if((strcmp(value, "add") == 0)||(strcmp(value, "remove") == 0)){
                if (!hasUpdated) updateCameraNum();
                ALOGI("CameraService::getNumberOfCameras() = %d",mNumberOfCameras);
            } else
                mNumberOfNormalCameras = mModule->getNumberOfCameras();
    ...
    property_get("sys.camera.hasHdmiIN", value, "0");
    ALOGD("sys.camera.hasHdmiIN:%s", value);
    if (!strcmp(value, "1")) {
        memset(value,0x00, sizeof(value));
        property_get("sys.camera.callprocess", value, "");
        ALOGD("sys.camera.callprocess: %s", value);
        if (strcmp(value, "teaonly.rk.droidipcam") && strcmp(value, "/system/bin/cameraserver"))
            *numCameras = mNumberOfCameras - 1;
    }
    return Status::ok();
}
...
void CameraService::doUserSwitch(const std::vector<int32_t>& newUserIds) {
    ...
    for (auto& i : mActiveClientManager.getAll()) {
        ...
        if (clientSp.get() == nullptr) {
            ALOGE("%s: Dead client still in mActiveClientManager.1", __FUNCTION__);
            continue;
        }
